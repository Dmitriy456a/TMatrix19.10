...
using TDynamicVector<TDynamicVector<T>>::sz; public:
    TDynamicMatrix(size_t s = 1) : 
        TDynamicVector<TDynamicVector<T>>(s)
    {
        if (s == 0)             
            throw out_of_range("Matrix size should be greater than zero"); if (s > MAX_MATRIX_SIZE)             
    ...
    {
        if (sz != m.sz) throw logic_error("Wrong dimensions"); 
        else {
            TDynamicMatrix result(sz);             
            for (size_t i = 0; i < sz; i++) 
